...
Te = 1.831                              # Electron Temperature [eV] @ np: 1.0E15 [1/m3]
...
Tp = 300
Vp = -1.0672723006747082            # Particle potential @ np: 1.0E15 [1/m3]
t = np.array([2870683734555531.0])             # particle density [1/m3]
...
def attach(Vp, x):
    ...
    for i in x:
        if i > -Vp:
            ...
            result.append(0)
    return result  
plt.plot(energy, elas(Vp,lam_f,energy))
plt.plot(energy, attach(Vp,energy))
plt.xlabel('Energy [eV]')
plt.ylabel('Cross Section [m2]')
plt.legend(['Effective Cross Section', 'Attachment Cross Section'])
fig = plt.gcf()
plt.grid(True)
